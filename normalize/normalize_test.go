...
func TestNormalizeValueNilValueIsNoOp(t *testing.T) {
	...
	r := newRewriter(nilSchemaIndex{})
	require.NoError(t, r.normalizeValue("p", nil, "", graphQLTypesFromSchema))
...
func TestNormalizeScalarValueStrictMissingTypeErrors(t *testing.T) {
	...
	r := newRewriter(nilSchemaIndex{})
	err := r.normalizeScalarValue("p", &ast.Value{Kind: ast.IntValue, Raw: "1"}, "", graphQLTypesFromSchema)
	require.ErrorIs(t, err, ErrGraphQLTypeUnresolved)
}
...
func TestNormalizeExistingVariableStrictMissingTypeErrors(t *testing.T) {
	...
	r := newRewriter(nilSchemaIndex{})
	err := r.normalizeExistingVariable("p", &ast.Value{Kind: ast.Variable, Raw: "x"}, "", graphQLTypesFromSchema)
	require.ErrorIs(t, err, ErrGraphQLTypeUnresolved)
}
...
func TestNormalizeListLiteralsStrictMissingTypeErrors(t *testing.T) {
	...
	require.ErrorIs(t, r.normalizeListLiterals("p", v, "", graphQLTypesFromSchema), ErrGraphQLTypeUnresolved)
}
